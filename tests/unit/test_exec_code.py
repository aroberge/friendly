...
def test_exec_code():
    ...
    assert not friendly.editors_helpers.exec_code(source=bad_code_syntax)
    result = friendly.get_output()  # content is flushed
    assert "SyntaxError" in result

    assert not friendly.get_output()  # confirm that content was flushed

    friendly.editors_helpers.exec_code(source=bad_code_exec)
    result = friendly.get_output()
    assert "NameError" in result

    assert friendly.editors_helpers.exec_code(source=good_code)
    ...
    friendly.uninstall()
    friendly.editors_helpers.exec_code(source=bad_code_syntax)
    assert not friendly.is_installed()
    friendly.editors_helpers.exec_code(source=bad_code_syntax, include="no_tb")
    assert not friendly.is_installed()
    ...
    friendly.editors_helpers.exec_code(source=bad_code_exec, lang="fr", include="explain")
    result = friendly.get_output()
